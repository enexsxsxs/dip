<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">Вид оборудования (категория)</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl shadow-lg border border-teal-100 overflow-hidden">
            <div class="p-6 border-b border-slate-200">
                <form method="post" action="{{ route('equipment-types.store') }}" class="flex flex-wrap items-end gap-3">
                    @csrf
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-200">
                            ...
                            <tr class="hover:bg-teal-50/50">
                                ...
                                <td class="px-4 py-3 text-right">
                                    @if($item->equipment_count > 0)
                                        <span class="text-xs text-slate-500" title="Нельзя удалить: к этому виду привязано {{ $item->equipment_count }} ед. оборудования">Привязано оборудование ({{ $item->equipment_count }})</span>
                                    @else
                                        <form method="post" action="{{ route('equipment-types.destroy', $item) }}" class="inline" onsubmit="return confirm('Удалить этот вид оборудования?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-4 py-8 text-center text-slate-500 text-sm">Пока нет записей. Добавьте первый вид оборудования.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
