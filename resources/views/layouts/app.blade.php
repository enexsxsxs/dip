...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>body { font-family: 'Source Sans 3', ui-sans-serif, system-ui, sans-serif; }</style>
    </head>
    <body class="antialiased min-h-screen bg-[#a8bcc9]">
        ...
        <main class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 min-h-0 overflow-visible">
            {{ $slot }}
        </main>
        ...
        @if(session('success') || session('status') || session('deleted') || session('error'))
            <div class="fixed bottom-6 right-6 z-50 max-w-sm" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-8" x-transition:enter-end="opacity-100 translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                @if(session('success'))
                    <div class="rounded-xl px-5 py-4 shadow-lg border-2 border-teal-400 bg-teal-500 text-white font-medium text-sm flex items-center gap-3">
                ...
                @elseif(session('deleted'))
                    <div class="rounded-xl px-5 py-4 shadow-lg border-2 border-amber-400 bg-amber-500 text-white font-medium text-sm flex items-center gap-3">
                ...
                @elseif(session('error'))
                    <div class="rounded-xl px-5 py-4 shadow-lg border-2 border-red-400 bg-red-500 text-white font-medium text-sm flex items-center gap-3">
