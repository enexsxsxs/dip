<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-slate-800 leading-tight">Новая заявка по макету</h2>
    </x-slot>
    ...
    <div class="max-w-4xl bg-white/95 rounded-2xl shadow-sm border border-teal-100 p-6 sm:p-8"
         ...
         x-init="init()">
        <form method="post" action="{{ route('report-requests.store') }}" class="space-y-6" @submit="syncDataJson()">
            @csrf
            <div>
                ...
                <select name="request_layout_id" id="request_layout_id" required
                    ...
                    @foreach($layouts as $l)
                        <option value="{{ $l->id }}" @selected(old('request_layout_id') == $l->id)>{{ $l->title }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="recipient_user_id" id="recipient_user_id"
                    ...
                    @foreach($users as $u)
                        <option value="{{ $u->id }}" @selected(old('recipient_user_id') == $u->id)>{{ $u->name }} (id {{ $u->id }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex gap-4">
                <button type="submit" class="min-h-[48px] px-6 rounded-xl bg-teal-600 text-white font-semibold hover:bg-teal-700 transition">Создать</button>
                <a href="{{ route('report-requests.index') }}" class="inline-flex items-center min-h-[48px] px-6 rounded-xl border-2 border-slate-200 text-slate-700 font-semibold hover:bg-slate-50">Отмена</a>
            </div>
        </form>
    </div>
</x-app-layout>
